...
async function fetchAPI(payload) {
    try {
        ...
        if (data.status === 'success') {
            ...
            if (payload.action === 'read' && data.pesan.startsWith('NOT_FOUND::')) {
                const failedCmd = data.pesan.split('::')[1];
                startFakeLoading(failedCmd);
            } else {
...
let fakeLoadingId = null;

function startFakeLoading(cmd) {
    if (fakeLoadingId) return;

    const tasks = [
        'SCANNING DATABASE NODES',
        'CROSS-REFERENCING KEYWORD',
        'PROBING DEEP STORAGE',
        'TRACING ENCRYPTED INDEX',
        'ANALYZING NEURAL CACHE',
        'BYPASSING FIREWALL LAYER',
        'DECRYPTING SECTOR 0xFF'
    ];

    appendHistory(`> querying '${cmd}'...`, 'system-msg');

    let percent = 0;
    let task = tasks[Math.floor(Math.random() * tasks.length)];

    fakeLoadingId = setInterval(() => {
        percent += Math.floor(Math.random() * 8) + 3;
        if (percent >= 100) {
            percent = Math.floor(Math.random() * 15);
            task = tasks[Math.floor(Math.random() * tasks.length)];
        }
        const bars = '|'.repeat(percent);
        const dashes = '-'.repeat(100 - percent);
        const line = document.createElement('div');
        line.className = 'error-msg';
        line.textContent = `[${bars}${dashes}] ${String(percent).padStart(3, ' ')}% :: ${task}`;
